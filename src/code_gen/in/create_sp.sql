...
-- SELECT COUNT(*) <TARGET_SCHEMA>.<TARGET_TABLE> WITH(NOLOCK)
...
AS
	...
	IF (@PERIOD_START_DTTM IS NULL OR @PERIOD_END_DTTM IS NULL)
		BEGIN
			SELECT @CrntMAX_ID = max(ID)
			FROM <TARGET_SCHEMA>.<TARGET_TABLE> WITH (NOLOCK);

			IF (@CrntMAX_ID IS NULL)
				BEGIN
					SET @PERIOD_START_DTTM = '2000-01-01';
