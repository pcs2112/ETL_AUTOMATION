...
-- C8_<STORED_PROCEDURE_SCHEMA>.<STORED_PROCEDURE_NAME>_SP.sql
-- <DATE_CREATED>
-- Generated by:
...
-- SELECT * FROM [MWH].[ETL_CONTROL_MANAGER] WITH (NOLOCK) WHERE PROCEDURE_NAME = '<STORED_PROCEDURE_SCHEMA>.<STORED_PROCEDURE_NAME>'
...
AS
	...
	DECLARE @SET_DAY_START BIT = <SET_DAY_START>;
	...
	IF(@SET_DAY_START = 1)
		BEGIN
			SET @SPERIOD_START_DTTM = UMA_MSTR.GetDayStart(@SPERIOD_START_DTTM);
		END
...
<SOURCE_TABLE_COLUMN_NAMES>
	...
	WHERE <SOURCE_TABLE_SEARCH_CONDITION>;
...
<TARGET_TABLE_COLUMN_NAMES>
	...
	ON (<TARGET_TABLE_PRIMARY_KEY_CONDITION>)
	<MATCH_SECTION>
	UPDATE SET
