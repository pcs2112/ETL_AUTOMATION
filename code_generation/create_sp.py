    ...
    for column in table_definition:
        if column['column_name'] in match_check_columns:
            if column['is_nullable'] == 1:
                conditions.append(f"(target.{column['column_name']} is NULL and source.{column['column_name']} is NOT NULL)")
                conditions.append(f"(target.{column['column_name']} is NOT NULL and source.{column['column_name']} is NULL)")
