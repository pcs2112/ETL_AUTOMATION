IF EXISTS (SELECT * FROM sys.objects so JOIN  sys.schemas ss on (so.schema_id = ss.schema_id) WHERE so.type = 'P' AND so.name = '<STORED_PROCEDURE_NAME>' and ss.name = '<TARGET_SCHEMA>')
    DROP PROCEDURE <TARGET_SCHEMA>.<STORED_PROCEDURE_NAME>
GO

EXEC  MWH.ETL_SOURCE_TARGET_DETAIL_MGR  'ADD', '<SOURCE_DATA_MART>','<SOURCE_SERVER>','<SOURCE_DATABASE>','<SOURCE_SCHEMA>','<SOURCE_TABLE>','<SOURCE_TABLE_SEARCH_COLUMN>','<TARGET_SCHEMA>', '<TARGET_TABLE>','<SOURCE_TABLE_SEARCH_COLUMN>','<STORED_PROCEDURE_NAME>',1
go
...
EXEC  MWH.MNG_LOAD_HISTORY   'START', @START_DATETIME, 0 ,  @Source_Server_Name,  @Source_DB_Name,  @Source_Schema_Name,  @Source_Table_Name, @Target_Schema_Name, @Target_Table_Name, @My_SP_NAME, @vPERIOD_START_DTTM, @vPERIOD_END_DTTM, 0 , 0, 0 , 0 , 0, '',   @LOAD_HIST_PKID  OUTPUT;
...
EXEC  MWH.MNG_LOAD_HISTORY   'FINISHED', @END_DATETIME, @LOAD_HIST_PKID, @Source_Server_Name,  @Source_DB_Name,  @Source_Schema_Name,  @Source_Table_Name, @Target_Schema_Name, @Target_Table_Name, @My_SP_NAME, @vPERIOD_START_DTTM, @vPERIOD_END_DTTM, @rtn_Insert_Cnt , @rtn_Update_Cnt ,@rtn_Delete_Cnt , @TryCatchError_ID , 0, '', @LOAD_HIST_DUMMY  OUTPUT;
